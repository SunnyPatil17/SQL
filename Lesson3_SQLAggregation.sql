...
SELECT sum(standard_amt_usd) / sum(standard_qty) AS result
FROM orders ;
...
AVG(poster_qty) avg_poster_amt
FROM orders

/* Via the video, you might be interested in how to calculate the MEDIAN. Though this is more advanced than what we have covered so far try finding -
...
SELECT accounts.name as account_name, MIN(orders.occurred_at) as earliest_order_date
FROM accounts
JOIN orders
...
sum(orders.total) as total_sales_usd
...
SELECT s.name as salesrep, w.channel as channel, count(w.channel) as occurences
...
SELECT r.name as region, w.channel as channel, count(w.channel) as occurences
FROM region r JOIN sales_reps s
ON r.id = s.region_id
JOIN accounts a
ON s.id = a.sales_rep_id
...
SELECT COUNT(*) as salesrep_count
FROM
(SELECT s.id, s.name, COUNT(a.id) as account_count
FROM sales_reps s
JOIN accounts a
ON s.id = a.sales_rep_id
GROUP BY s.id, s.name
HAVING COUNT(a.id) > 5
ORDER BY account_count) as referencetable ;
...
SELECT o.account_id, a.name, COUNT(o.id) as order_count
...
SELECT o.account_id, a.name, SUM(o.total_amt_usd) as total_amt
FROM orders o JOIN accounts a
ON a.id = o.account_id
GROUP BY o.account_id, a.name
...
SUM(total_amt_usd) as dollars_amt
FROM orders
GROUP BY yr
ORDER BY dollars_amt DESC ;
...
DATE_PART('year',o.occurred_at) as yr,
SUM(o.gloss_qty) as glosspaper
FROM orders o JOIN accounts a
ON o.account_id = a.id
WHERE a.name = 'Walmart'
GROUP BY month, yr
ORDER BY glosspaper DESC
LIMIT 1;
...
SELECT
a.name customer, sum(o.total_amt_usd) total_spent,
  CASE
...
SELECT s.name sales_rep , count(o.id) total_order,
  CASE
...
SELECT s.name sales_rep, COUNT(*) total_order, SUM(o.total_amt_usd) total_spent,
     CASE
